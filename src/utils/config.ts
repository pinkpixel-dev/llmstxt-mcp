 ...
 */
export async function loadConfigFile(filePath: string, fileFormat: 'yaml' | 'json'): Promise<DocSource[]> {
  try {
...
export function createDocSourcesFromUrls(urls: string[]): DocSource[] {
  ...
  for (const entry of urls) {
    ...
    if (trimmedEntry.includes(':') && !trimmedEntry.startsWith('http:') && !trimmedEntry.startsWith('https:')) {
...
export async function parseConfig(options: OptionsType): Promise<ParsedConfig> {
  ...
  if (docSources.length === 0) {
    throw new Error('No documentation sources configured. Use --yaml, --json, or --urls options.');
  }
